...
const multiAssignmentMutation = useMutation(
    ...
    return (
        ...
        {assignments && Array.isArray(assignments) && assignments.map( (assignment, roleindex) => {
            ...
            {assignment.ID > 0 && 'Speaker' == attrs.role && <p className="evaluation-request"><a href={assignment.evaluation_link} onClick={(e) => {e.preventDefault(); setEvaluate(assignment);setMode('evaluation')}} >Evaluation Form</a> <span style={{fontSize: '10px'}}>(copy-paste text below to share)</span><br /><textarea rows="3" style={{fontSize: '8px'}} value={'Evaluation link for '+assignment.name+'\n'+assignment.evaluation_link} /></p>}
