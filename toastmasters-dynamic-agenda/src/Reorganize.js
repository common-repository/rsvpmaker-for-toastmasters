...
function selectMove(source,destination) {
    ...
    function CopyToTemplateButton () {
        return <button onClick={() => {data.copyToTemplate = true; agendaMutate(data)} }>Apply to All</button>
    }
    ...
    return (
        ...
        <div className="agendawrapper" id={"agendawrapper"+post_id}>
            ...
            {data.has_template && <div><p>By default, changes you make below will apply <em>only</em> to a single meeting.</p><p><CopyToTemplateButton /><br /><em>Click to copy the agenda changes you have made to all upcoming meetings based on the same template. The template will also be updated.</em></p></div>}
